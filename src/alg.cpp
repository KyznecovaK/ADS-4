...
int countPairs2(int *arr, int len, int value) {
 ...
 arr[слева] == arr[слева + leftCount]) {
 leftCount++;
 }

 в то время как (right - rightCount > left && 
...
int lowerBound(int *arr, int left, int right, int target) {
 int result = -1;
 while (слева <= справа) {
 int mid = слева + (справа - налево) / 2; 
 if (arr[mid] == цель) {
 результат = середина;
 справа = середина - 1;
 } else if (arr[mid] < цель) {
 слева = mid + 1;
 } else {
 right = mid - 1;
 }
...
int countPairs3(int *arr, int len, int value) {
 ...
 if (target == arr[i]) {
 int last = верхний предел (arr, i, len - 1, arr[i]);
 int cnt = last - i + 1;
 count += cnt * (cnt - 1) / 2;
 } else {
